...
export default function Category2Form() {
  ...
  const fetchCategoryData = async () => {
    try {
      const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:8000';
      const res = await axiosServices.get(`${apiUrl}/api/carbon/category/2`);
  ...
  const handleSave = async () => {
    ...
    try {
      const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:8000';
      const res = await axiosServices.post(`${apiUrl}/api/carbon/category/2`, { sites });
      if (res.data.category?.calculations) setCalculations(res.data.category.calculations);
      Swal.fire({
    ...
    <>
      ...
      {calculations && resultsReleased && (
        <MainCard className="mt-3">
          ...
          {showChart && <CategoryChart code="2" calculations={calculations} />}
