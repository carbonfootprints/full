...
export default function Category3_3_1Form() {
  ...
  const fetchCategoryData = async () => {
    try {
      const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:8000';
      const res = await axiosServices.get(`${apiUrl}/api/carbon/category/3.3.1`);
  ...
  const handleSave = async () => {
    ...
    try {
      const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:8000';
      const res = await axiosServices.post(`${apiUrl}/api/carbon/category/3.3.1`, { sites });
      if (res.data.category?.calculations) setCalculations(res.data.category.calculations);
      Swal.fire({
    ...
    <>
      ...
      <MainCard>
        <Row className="align-items-center">
          <Col>
            ...
            <h4 className="mb-1">Category 3.3.1 — Upstream Road Transportation (Chemicals)</h4>
      ...
      {calculations && resultsReleased && (
        <MainCard className="mt-3">
          ...
          {showChart && <CategoryChart code="3.3.1" calculations={calculations} />}
