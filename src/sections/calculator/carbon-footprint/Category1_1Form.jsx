...
export default function Category1_1Form() {
  ...
  const fetchCategoryData = async () => {
    ...
    try {
      ...
      const res = await axios.get(`${apiUrl}/api/carbon/category/1.1`, {
        headers: {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await axios.post(`${apiUrl}/api/carbon/category/1.1`, { sites }, { headers: { Authorization: `Bearer ${token}` } });
      ...
      Swal.fire({
        ...
        text: `Category 1.1 data saved. Total CO2e: ${res.data.category?.grandTotals?.totalCO2e?.toFixed(3) ?? '—'} kgCO2e`,
        timer: 2500
      });
      return true;
    } catch (error) {
      ...
      {calculations && (
        <MainCard className="mt-3">
          <h5 className="mb-3">
            ...
            <Badge bg="success" className="ms-2 px-3">
              Category 1.1
            </Badge>
          </h5>
