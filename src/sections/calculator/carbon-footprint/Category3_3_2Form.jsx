...
export default function Category3_3_2Form() {
  ...
  const fetchCategoryData = async () => {
    try {
      const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:8000';
      const res = await axiosServices.get(`${apiUrl}/api/carbon/category/3.3.2`);
  ...
  const handleSave = async () => {
    ...
    try {
      const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:8000';
      const res = await axiosServices.post(`${apiUrl}/api/carbon/category/3.3.2`, { sites });
      if (res.data.category?.calculations) setCalculations(res.data.category.calculations);
      Swal.fire({
        icon: 'success', title: 'Data Saved',
        text: 'Category 3.3.2 data saved successfully. Results will be available once reviewed by the administrator.',
        timer: 2500
      });
      return true;
    } catch (error) {
  ...
  const handleSaveAndNext = async () => { if (await handleSave()) navigate('/calculate/carbon-footprint/category/3.4.1'); };
      ...
      <MainCard>
        <Row className="align-items-center">
          <Col>
            ...
            <h4 className="mb-1">Category 3.3.2 - Upstream Road Transportation (Chemicals to Thirumudivakkam)</h4>
                      ...
                      {(site.consignments || []).map((c, ri) => {
                        ...
                        return (
                          <tr key={ri}>
                            ...
                            <td>
                              <Form.Control size="sm" type="text" placeholder="e.g., M.A International"
                                value={c.companyName || ''}
                                onChange={(e) => updateRow(si, ri, 'companyName', e.target.value)} />
                            </td>
                            <td>
                              <Form.Control size="sm" type="text" placeholder="e.g., Filter UV"
                            ...
                            <td>
                              <Form.Control size="sm" type="text" placeholder="e.g., Pammal"
        ...
        <MainCard className="mt-3">
          <Stack direction="horizontal" className="justify-content-between align-items-center mb-3">
            <h5 className="mb-0">
              ...
              <Badge bg="success" className="ms-2 px-3">Category 3.3.2</Badge>
            </h5>
            <Button
          ...
          {showChart && <CategoryChart code="3.3.2" calculations={calculations} />}
          ...
          <Card className="border-success">
            <Card.Header className="bg-success text-white fw-semibold">Grand Total — All Sites (Category 3.3.2)</Card.Header>
            <Card.Body className="p-0">
              <Table bordered size="sm" className="mb-0">
                <thead className="table-light">
                  <tr>
