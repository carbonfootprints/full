...
export default function Category3_4_2Form() {
  ...
  const fetchCategoryData = async () => {
    try {
      const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:8000';
      const res = await axiosServices.get(`${apiUrl}/api/carbon/category/3.4.2`);
  ...
  const handleSave = async () => {
    ...
    try {
      const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:8000';
      const res = await axiosServices.post(`${apiUrl}/api/carbon/category/3.4.2`, { sites });
      if (res.data.category?.calculations) setCalculations(res.data.category.calculations);
      Swal.fire({
  ...
  const handleSaveAndNext = async () => { if (await handleSave()) navigate('/calculate/carbon-footprint/category/3.5.1'); };
      ...
      <MainCard>
        <Row className="align-items-center">
          <Col>
            ...
            <h4 className="mb-1">Category 3.4.2 - Upstream Sea Transportation (Chemicals — Italy Imports)</h4>
                      ...
                      {(site.consignments || []).map((c, ri) => {
                        ...
                        return (
                          <tr key={ri}>
                            ...
                            <td>
                              <Form.Control size="sm" type="text" placeholder="e.g., Multikimica"
                                value={c.supplier || ''}
                                onChange={(e) => updateRow(si, ri, 'supplier', e.target.value)} />
                            </td>
                            <td>
                              <Form.Control size="sm" type="text" placeholder="e.g., Oil Bo"
        ...
        <MainCard className="mt-3">
          <Stack direction="horizontal" className="justify-content-between align-items-center mb-3">
            <h5 className="mb-0">
              ...
              <Badge bg="success" className="ms-2 px-3">Category 3.4.2</Badge>
            </h5>
            <Button
          ...
          {showChart && <CategoryChart code="3.4.2" calculations={calculations} />}
          ...
          <Card className="border-success">
            <Card.Header className="bg-success text-white fw-semibold">Grand Total — All Sites (Category 3.4.2)</Card.Header>
            <Card.Body className="p-0">
              <Table bordered size="sm" className="mb-0">
                <thead className="table-light">
                  <tr>
