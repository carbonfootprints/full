...
// LCA-based carbon intensity for leather chemicals
...
export default function Category4_2aForm() {
  ...
  const fetchExisting = async () => {
    try {
      const res = await axiosServices.get(`${apiUrl}/api/carbon/category/4.2.a`);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await axiosServices.post(`${apiUrl}/api/carbon/category/4.2.a`, payload);

      await Swal.fire({
        icon: 'success',
        title: 'Data Saved',
        text: `Category 4.2.a upstream chemicals data saved. Total CO₂e: ${grandTotal.toFixed(4)} kg`,
  ...
  const handleClear = async () => {
    const result = await Swal.fire({
      icon: 'warning',
      title: 'Clear Data?',
      text: 'This will permanently delete all Category 4.2.a upstream chemicals data.',
    ...
    <>
      ...
      <MainCard className="mt-3">
        <Alert variant="info" className="mb-0 small">
          ...
          <strong>Formula:</strong> Sum of all chemical quantities (kg) × {CHEMICAL_EF} kg CO₂e/kg
          &nbsp;·&nbsp; EF source: LCA-based carbon intensity for leather processing chemicals.
        </Alert>
      </MainCard>
          ...
          <Card className="mt-3" key={sIdx}>
            <Card.Header className="d-flex justify-content-between align-items-center">
              <div className="d-flex align-items-center gap-3 flex-grow-1">
                <h5 className="mb-0">
                  <i className="ph ph-factory me-2 text-primary" />
                  Site {sIdx + 1}
                </h5>
                <Form.Control
                  size="sm"
            ...
            <Card.Body>
                    ...
                    {site.chemicals.map((row, rIdx) => (
                      <tr key={rIdx}>
                        <td>
                          <Form.Control
                            size="sm"
                            placeholder="e.g. Chrome sulphate"
                            value={row.chemicalName}
                            onChange={(e) => updateRow(sIdx, rIdx, 'chemicalName', e.target.value)}
                          />
                        </td>
                        <td>
                          <Form.Control
              ...
              <Button variant="outline-primary" size="sm" onClick={() => addRow(sIdx)}>
