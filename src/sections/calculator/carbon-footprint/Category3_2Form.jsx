...
export default function Category3_2Form() {
  ...
  const fetchCategoryData = async () => {
    try {
      const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:8000';
      const res = await axiosServices.get(`${apiUrl}/api/carbon/category/3.2`);
  ...
  const handleSave = async () => {
    ...
    try {
      const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:8000';
      const res = await axiosServices.post(`${apiUrl}/api/carbon/category/3.2`, { sites });
      if (res.data.category?.calculations) setCalculations(res.data.category.calculations);
      Swal.fire({
        icon: 'success', title: 'Data Saved',
        text: 'Category 3.2 data saved successfully. Results will be available once reviewed by the administrator.',
        timer: 2500
      });
      return true;
    } catch (error) {
  ...
  const handleSaveAndNext = async () => { if (await handleSave()) navigate('/calculate/carbon-footprint/category/3.3.1'); };
      ...
      <MainCard>
        <Row className="align-items-center">
          <Col>
            ...
            <h4 className="mb-1">Category 3.2 - Upstream Road Transportation (Tanned Leather)</h4>
            <p className="text-muted mb-0">Incoming consignment data — weight × distance = t·km → CO₂e</p>
          </Col>
          <Col xs="auto">
            <Badge bg="secondary" className="px-3 py-2"><i className="ph ph-truck me-2" />Scope 3</Badge>
          </Col>
        </Row>
      </MainCard>
                  ...
                  <Table bordered hover size="sm">
                    <thead className="table-light">
                      <tr>
                        ...
                        <th style={{ minWidth: 130 }}>Weight (kg)</th>
                        <th style={{ minWidth: 150 }}>Supplier Location</th>
                        <th style={{ minWidth: 120 }}>Distance (km)</th>
                        <th className="text-end" style={{ minWidth: 110 }}>t·km</th>
                        <th className="text-end" style={{ minWidth: 120 }}>CO₂e (kg)</th>
                        <th></th>
                      </tr>
                    </thead>
                    <tbody>
                      {(site.consignments || []).map((c, ri) => {
                        ...
                        return (
                          <tr key={ri}>
                            ...
                            <td>
                              <Form.Control size="sm" type="text" placeholder="e.g., INV-001"
                                value={c.invoiceNo || ''}
                            ...
                            <td>
                              <Form.Control size="sm" type="text" placeholder="e.g., Erode"
        ...
        <MainCard className="mt-3">
          <Stack direction="horizontal" className="justify-content-between align-items-center mb-3">
            <h5 className="mb-0">
              ...
              <Badge bg="success" className="ms-2 px-3">Category 3.2</Badge>
            </h5>
            <Button
          ...
          {showChart && <CategoryChart code="3.2" calculations={calculations} />}
          ...
          <Card className="border-success">
            <Card.Header className="bg-success text-white fw-semibold">Grand Total — All Sites (Category 3.2)</Card.Header>
            <Card.Body className="p-0">
              <Table bordered size="sm" className="mb-0">
                <thead className="table-light">
                  <tr>
