...
function ReactTable({ defaultColumns, defaultData }) {
  ...
  table.getAllColumns().map((column) =>
    headers.push({
      ...
      key: 'accessorKey' in column.columnDef ? column.columnDef.accessorKey : column.id
    })
  );
              ...
              <DraggableRow key={row.id} row={row} reorderRow={reorderRow}>
                <>
                  {row.getVisibleCells().map((cell) => (
                    <td key={cell.id} {...cell.column.columnDef.meta} className={`${cell.column.columnDef.meta?.className} `}>
