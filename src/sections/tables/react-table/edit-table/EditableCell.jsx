...
function TanStackTable({ columns, initialData, striped }) {
  const [data, setData] = useState(initialData);
  const [editing, setEditing] = useState({});
  const [formData, setFormData] = useState({});

  const handleEditToggle = (field, rowIndex) => {
    setEditing((prev) => ({
      ...prev,
  ...
  const handleFieldChange = (field, value, rowIndex) => {
    setFormData((prev) => ({
      ...prev,
  ...
  const handleSave = (field, rowIndex) => {
          ...
          {data.map((row, rowIndex) => (
            <tr key={rowIndex}>
              {columns.map((column) => {
                ...
                const currentValue = formData[`${field}-${rowIndex}`] ?? row[field];
                    ...
                    {editing[`${field}-${rowIndex}`] ? (
                      field === 'status' ? (
                        ...
                        <Slider
                          ...
                          onChange={(value) => handleFieldChange(field, value, rowIndex)}
                      ...
                      <span onClick={() => handleEditToggle(field, rowIndex)}>{row[field]}</span>
                    )}
                  </td>
                );
              })}
...
export default function EditableCellTable({ striped }) {
...
TanStackTable.propTypes = { columns: PropTypes.array, initialData: PropTypes.array, striped: PropTypes.bool };

EditableCellTable.propTypes = { striped: PropTypes.bool, title: PropTypes.string };
